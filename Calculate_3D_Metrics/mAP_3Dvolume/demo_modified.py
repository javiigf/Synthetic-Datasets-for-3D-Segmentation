...
def load_data(args, slices):
    ...
    if args.predict_score != '':
        ...
        if '.h5' in args.predict_score:
            ...
            pred_score = pred_score[pred_score[:, 0] != 0]  # eliminar fondo por si acaso
            pred_score[:, 0] = pred_score[:, 0].astype(int)  # <- esta es la clave
        else:
    ...
    areaRng = np.zeros((len(thres)+2,2),dtype=np.float64)
    areaRng[0,1] = 1e10
    areaRng[-1,1] = 1e10
    areaRng[2:,0] = thres
    areaRng[1:-1,1] = thres
...
def main(_args):
    ...
    print('\t2. Compute IoU')
    pred_score[:, 0] = pred_score[:, 0].astype(int)
