...
predictedPath = "/media/pedro/6TB/jesus/EM_Image_Segmentation/exp_results/cyst39_v2_BCM/results/cyst39_v2_BCM_5/per_image_instances/"
groundTruthPath = "/media/pedro/6TB/jesus/methodology_naturalVariation/dataset_DANI/treeOrganization_test/test/y/"
 #   """Calcualte over-segmentation and under-segmentation rates based
 #    on the paper entitled "Assessment of deep learning algorithms
...
fileName = "4d.3B.3_3_norm1.tif"
fileName2 = "4d.3B.3_3_norm1.tif"
...
#Number of labels
groundTruthLabelsNum = np.size(np.unique(groundTruthLabels))
predictedCellsNum = np.size(np.unique(predictedLabels))
...
groundTruthLabels = relabel_sequential(groundTruthLabels, 1)[0]-1
predictedLabels = relabel_sequential(predictedLabels, 1)[0]-1
...
for groundTruthCell in groundTruthCells:
    ...
    groundTruthCellVolume[groundTruthCell] = np.count_nonzero(groundTruthLabels[validSlices, :, :]==groundTruthCell)
    matchingCells = np.unique(predictedLabels[(groundTruthLabels==groundTruthCell)])

    for predictedCell in matchingCells:
        
        pCell =  predictedLabels[validSlices, :, :]==predictedCell
